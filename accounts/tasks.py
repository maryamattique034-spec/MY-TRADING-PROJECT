...
from .models import LedgerEntry, Order, Stock, TradingPosition
...
def process_order(order_id):
    try:
        ...
        logger.info(
            f"Starting process for Order #{order.id} ({order.order_type})-"
        ...
        with transaction.atomic():
            if order.order_type == "BUY":
                if account.balance >= total_cost:
                    ...
                    logger.info(
                        f"Account #{account.id} balance updated:{old_balance} -> {account.balance}"
                    )

                    LedgerEntry.objects.create(
                        account=account, transaction_type="withdraw", amount=total_cost
                    )
                    logger.info(
                        f"LedgerEntry created for withdrawal of {total_cost} for Account #{account.id}"
                    )
                    ...
                    position.save()
                    logger.info(
                        f"Trading_Position updated for {stock.ticker}: Quantity= {position.quantity}, Avg_Price = {position.avg_price}"
                    )

                    order.price = stock.price
                    order.status = "COMPLETED"
                    logger.info(
                        f"Order #{order.id} marked COMPLETED at price {order.price}"
                    )
                else:
                    order.status = "CANCELLED"
                    logger.warning(
                        f"Order #{order.id} cancelled - Insufficient balance in Account #{account.id}"
                    )

            elif order.order_type == "SELL":
                try:
                    position = TradingPosition.objects.get(
                        account=account, stock_ticker=stock.ticker
                    )
                except TradingPosition.DoesNotExist:
                    order.status = "CANCELLED"
                    order.save()
                    logger.warning(
                        f"Order #{order.id} cancelled - No position found for {stock.ticker}"
                    )
                    return
                ...
                if position.quantity >= order.quantity:
                    ...
                    if position.quantity == 0:
                        position.delete()
                        logger.info(
                            f"Position for {stock.ticker} deleted (all shares sold)"
                        )
                    else:
                        position.save()
                        logger.info(
                            f"Position for {stock.ticker} updated: {old_quantity} -> {position.quantity}"
                        )
                    ...
                    account.save()
                    logger.info(
                        f"  #{account.id} balance updated: {old_balance} -> {account.balance}"
                    )

                    LedgerEntry.objects.create(
                        account=account, transaction_type="deposit", amount=total_cost
                    )
                    logger.info(
                        f"LedgerEntry created for deposit of {total_cost} for Account #{account.id}"
                    )
                ...
                order.status = "CANCELLED"
                logger.warning(
                    f"Order #{order.id} cancelled -Invalid order type {order.order_type}"
                )
