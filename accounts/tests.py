...
from .models import CustomUser, TradingAccount, TradingPosition, LedgerEntry,Stock
...
class AccountPositionsTests(APITestCase):
    def setUp(self):
        ...
        TradingPosition.objects.create(account = self.account, stock_ticker = "AAPL", quantity = 10, avg_price="150.00")
        TradingPosition.objects.create(account=self.account, stock_ticker="TSLA", quantity=5, avg_price="650.00")
        LedgerEntry.objects.create(account = self.account, transaction_type = "deposit", amount = 1500)
        LedgerEntry.objects.create(account = self.account, transaction_type = "withdraw", amount = 200)
...
class StockListViewTests(APITestCase):
    def setUp(self):
        ...
        Stock.objects.create(ticker = "AAPL", exchange = "NASDAQ", name = "Apple Inc.", price = 100)
...
class StockBulkUploadTests(APITestCase):
    def setUp(self):
        self.admin = CustomUser.objects.create_superuser(username="admin", email= "", password="1234")
        self.user = CustomUser.objects.create_user(username="user", email = "", password= "1234")
        TradingAccount.objects.create(user=self.user)
        self.url = reverse('stock_list_create')
        self.data = [
            {'ticker':'GOOGL', 'exchange':'NASDAQ', 'name':'Google LLC', 'price':2800},
    ...
    def test_bulk_upload_duplicated_admin(self):
        ...
        self.data = [
            ...
            {'ticker': 'TSLA', 'exchange': 'NASDAQ', 'name': 'duplicate_tesla', 'price': 330}
        ]
        response=self.client.post(self.url, self.data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class StockRUDTests(APITestCase):
    def setUp(self):
        ...
        self.user = CustomUser.objects.create_user(username="user", email="", password="1234")
        TradingAccount.objects.create(user=self.user)
        self.stock = Stock.objects.create(ticker="TSLA", exchange="NASDAQ", name="Tesla", price=500)
...
class StockRetreiveByTickerTests(APITestCase):
    def setUp(self):
        ...
        TradingAccount.objects.create(user=self.user)
        self.stock = Stock.objects.create(ticker = 'AAPL', exchange = 'NASDAQ', name = 'Apple Inc.', price = 250)
